...
from Q1_pour3d import relaxation, init_conditions, scale, Nx, Ny, a, b, c, d, e, f
...
def get_dynodes_ordonnes(scale, a, b, c, d, e, f):
  ...
  # dynode 3 : haut droite
  x3 = int(a + (1.5) * (c + d))
  dynodes_ordonnees.append({'x': x3, 'y': y_haut, 'c': c, 'e': e})


  x4 = int(a + (2) * (c + d))
  dynodes_ordonnees.append({'x': x4, 'y': y_bas, 'c': c, 'e': e})


  x5 = int(a + (2.5) * (c + d))
  dynodes_ordonnees.append({'x': x5, 'y': y_haut, 'c': c, 'e': e})


  x6 = int(a + (3) * (c + d))
  dynodes_ordonnees.append({'x': x6, 'y': y_bas, 'c': c, 'e': e})


  x7 = int(a + (3.5) * (c + d))
  dynodes_ordonnees.append({'x': x7, 'y': y_haut, 'c': c, 'e': e})


  x8 = int(a + (4) * (c + d))
  dynodes_ordonnees.append({'x': x8, 'y': y_bas, 'c': c, 'e': e})


  x9 = int(a + (4.5) * (c + d))
  dynodes_ordonnees.append({'x': x9, 'y': y_haut, 'c': c, 'e': e})


  x10 = int(a + (5) * (c + d))
  dynodes_ordonnees.append({'x': x10, 'y': y_bas, 'c': c, 'e': e})


  x11 = int(a + (5.5) * (c + d))
  dynodes_ordonnees.append({'x': x11, 'y': y_haut, 'c': c, 'e': e})
...
for step in range(nb_steps):
  ...
  # Vérification d'impact avec la dynode actuelle
  if dynode_idx < len(dynodes_sequence):
      ...
      if electron_touche_dynode(x, y, d):
          ...
          vy = 0
          dynode_idx += 1  # passer à la suivante
