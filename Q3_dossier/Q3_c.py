import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

import numpy as np
import matplotlib.pyplot as plt
from scipy.interpolate import RegularGridInterpolator
from Q1_pour3c import relaxation, init_conditions, scale, Nx, Ny, a, b, c, d, e, f
from Q2_pour3c import calcul_champ_electrique

# Constantes physiques
e_charge = -1.602e-19
m = 9.109e-31

# Simulation
dt = 3e-11
duree_totale = 3e-7
nb_steps = int(duree_totale / dt)

# Position initiale
x0_mm = 0.0
y0_mm = f / (2 * scale)  # départ centré
vx0, vy0 = 0.0, 0.0

x0 = x0_mm * scale
y0 = y0_mm * scale

# Potentiel et champ
V = np.zeros((Ny, Nx))
V = init_conditions(V)
V = relaxation(V)
Ex, Ey = calcul_champ_electrique(V)

# Interpolateurs
x_vals = np.arange(Nx)
y_vals = np.arange(Ny)
interp_Ex = RegularGridInterpolator((y_vals, x_vals), Ex, bounds_error=False, fill_value=0)
interp_Ey = RegularGridInterpolator((y_vals, x_vals), Ey, bounds_error=False, fill_value=0)

# Création des dynodes dans l’ordre attendu
def get_dynodes_ordonnes(scale, a, b, c, d, e, f):
    y_center = f // 2
    y_bas = int(y_center - 2 * scale - e // 2)
    y_haut = int(y_center + 2 * scale - e // 2)

    dynodes_ordonnees = []
    dynodes_ordonnees.append({'x': int(a + 0 * (c + d)), 'y': y_bas, 'c': c, 'e': e})
    dynodes_ordonnees.append({'x': int(a + 0.5 * (c + d)), 'y': y_haut, 'c': c, 'e': e})
    dynodes_ordonnees.append({'x': int(a + 1 * (c + d)), 'y': y_bas, 'c': c, 'e': e})
    dynodes_ordonnees.append({'x': int(a + 1.5 * (c + d)), 'y': y_haut, 'c': c, 'e': e})

    return dynodes_ordonnees

dynodes_sequence = get_dynodes_ordonnes(scale, a, b, c, d, e, f)
dynode_idx = 0  # dynode à viser

# Rebond vertical de 2 mm
rebond_pixels = 2 * scale

# Initialisation
positions = [(x0, y0)]
x, y = x0, y0
vx, vy = vx0, vy0
x_prev, y_prev = x, y  # pour détecter les franchissements

for step in range(nb_steps):
    if not (0 <= x < Nx and 0 <= y < Ny):
        break

    # Champ électrique local
    E = np.array([interp_Ex((y, x)), interp_Ey((y, x))])
    ax = (e_charge / m) * E[0] * scale * 1e3
    ay = (e_charge / m) * E[1] * scale * 1e3

    # Mémoriser la position précédente
    x_prev, y_prev = x, y

    # Intégration
    vx += ax * dt
    vy += ay * dt
    x += vx * dt
    y += vy * dt

    x = min(max(x, 0), Nx - 1)
    y = min(max(y, 0), Ny - 1)

    # Détection du passage par la face avant
    if dynode_idx < len(dynodes_sequence):
        d = dynodes_sequence[dynode_idx]
        x_face_avant = d['x']
        y_min = d['y']
        y_max = d['y'] + d['e']

        # Détection de la face avant de la dynode
        if abs(x - x_face_avant) < 1.0 and y_min <= y <= y_max:
            # Forcer la position à la face avant
            x = x_face_avant

            # Rebond vers le haut si dynode est en bas, vers le bas sinon
            y_centre_dynode = (y_min + y_max) / 2
            direction = +1 if y < y_centre_dynode else -1

            vx = 0
            vy = direction * (rebond_pixels / dt)

            dynode_idx += 1



    positions.append((x, y))

# Conversion
positions = np.array(positions) / scale

# Affichage
plt.figure(figsize=(10, 5))
plt.imshow(V, cmap="inferno", origin="lower", extent=[0, Nx/scale, 0, Ny/scale])
plt.colorbar(label="Potentiel (V)")
plt.plot(positions[:, 0], positions[:, 1], color="cyan", lw=1.5, label="Trajectoire")
plt.scatter([x0_mm], [y0_mm], color="cyan", label="Départ", zorder=5)
plt.title("Trajectoire de l’électron (rebond face avant des dynodes)")
plt.xlabel("x (mm)")
plt.ylabel("y (mm)")
plt.legend()
plt.tight_layout()
plt.savefig("figures_dos/trajectoire_ordonnée_3c.png")
plt.show()
